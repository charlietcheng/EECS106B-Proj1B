...
def vee(se3):
    v = se3[3][:3]
    w = np.array([se3[1][2], se3[2][0], se3[0][1]])
    twist = np.hstack((v,w))
    twist = twist.reshape(6,1)
    return twist

def log(xi): 
    "takes in xi hat and rerturns log xi hat "
    w_hat = xi[0:3,:3]
    v = xi[0:3,3]
    w = np.array([xi[1][2], xi[2][0],xi[0][1]])
    Ainv = np.eye(3) - (1/2) * w_hat +( (2 * sin(length(w))) - (length(w) * (1 + cos(length(w))))) * (w_hat **2) / (2 * length(w)**2 * sin(length(w))) 
    print("A:", Ainv@v)
    print("e", w_hat)
    tp_row = np.vstack((w_hat, Ainv @  v))
    bot = np.array([[0,0,0,0]])
    print(tp_row)
    print(bot)
    return np.hstack((tp_row,bot))
