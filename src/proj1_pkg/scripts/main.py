...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    if task == 'line':
        trajectory = LinearTrajectory([[current_position[0], current_position[1], current_position[2]], [tag_pos[0][0],tag_pos[0][1], tag_pos[0][2]+0.5]], 10)
    elif task == 'circle':
        print('executing before', current_position)
        trajectory = CircularTrajectory([tag_pos[0][0],tag_pos[0][1], tag_pos[0][2]+0.5], 0.2, 2*np.pi)
        # trajectory = CircularTrajectory([current_position[0],current_position[1], current_position[2]], 0.1, 2*np.pi)
        print('executing after', current_position)
    elif task == 'polygon':
        trajectory = PolygonalTrajectory([[tag_pos[0][0], tag_pos[0][1], tag_pos[0][2]+0.5], [tag_pos[1][0], tag_pos[1][1], tag_pos[1][2]+0.5], [tag_pos[2][0], tag_pos[2][1], tag_pos[2][2]+0.5, ], [tag_pos[0][0], tag_pos[0][1], tag_pos[0][2]+0.5]], 10)
    else:
